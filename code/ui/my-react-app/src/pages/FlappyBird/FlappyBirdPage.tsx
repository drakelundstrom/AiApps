...
import type { BirdState, GameState, Pipe } from './interfaces';
import { BIRD_SIZE, BIRD_X, GAME_HEIGHT, GAME_WIDTH, GRAVITY, JUMP_FORCE, PIPE_GAP, PIPE_SPEED, PIPE_SPAWN_INTERVAL, PIPE_WIDTH } from './constants/gameConfig'
...
function FlappyBird() {
  ...
  const birdRef = useRef<BirdState>({ y: GAME_HEIGHT / 2, velocity: 0 });
  ...
  useEffect(() => {
    ...
    const drawCloud = (cloudCtx: CanvasRenderingContext2D, x: number, y: number) => {
      cloudCtx.beginPath();
      cloudCtx.arc(x, y, 20, 0, Math.PI * 2);
      cloudCtx.arc(x + 25, y - 10, 25, 0, Math.PI * 2);
      cloudCtx.arc(x + 50, y, 20, 0, Math.PI * 2);
      cloudCtx.arc(x + 25, y + 5, 18, 0, Math.PI * 2);
      cloudCtx.fill();
    };
    ...
    const checkCollision = (bird: BirdState, pipes: Pipe[]) => {
    ...
    const gameLoop = (timestamp: number) => {
      ...
      if (state === 'playing') {
        ...
        // Update pipes
        pipesRef.current = pipesRef.current.filter((pipe: Pipe) => {
