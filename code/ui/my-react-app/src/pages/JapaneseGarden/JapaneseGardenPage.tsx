...
function SakuraPetals({ count = 300 }: SakuraPetalsProps) {
  const meshRef = useRef<THREE.InstancedMesh | null>(null)
  const dummy = useMemo(() => new THREE.Object3D(), [])

  const particles = useMemo(() => {
    const arr: SakuraPetalParticle[] = []
  ...
  return (
    <instancedMesh ref={meshRef} args={[petalShape, undefined, count]}>
...
function SakuraTree({ position = [0, 0, 0], scale = 1 }: PositionedScaledProps) {
  const trunkColor = '#5c3a1e'
  const canopyColor = '#ff8faa'
  const canopyClusters: Array<[number, number, number, number]> = [
    [0, 3.5, 0, 1.2],
    [-0.8, 3.2, 0.3, 0.8],
    [0.7, 3.4, -0.2, 0.7],
    [0, 3.9, 0.3, 0.6],
...
function StoneLantern({ position = [0, 0, 0] }: PositionedProps) {
  const stone = '#8a8a7a'
  return (
...
function ToriiGate({ position = [0, 0, 0] }: PositionedProps) {
  const red = '#c41e1e'
  return (
    <group position={position}>
      {/* left pillar */}
...
function KoiPond({ position = [0, 0, 0] }: PositionedProps) {
  return (
    <group position={position}>
      {/* water surface */}
...
function ZenGravel({ position = [0, 0, 0] }: PositionedProps) {
  return (
    <group position={position}>
...
function BambooCluster({ position = [0, 0, 0] }: PositionedProps) {
  return (
    <group position={position}>
...
function GardenScene({ showPetals }: GardenSceneProps) {
  return (
    <>
...
export default function JapaneseGarden() {
  ...
  return (
    <div style={{ width: '100%', height: '100dvh', position: 'relative', background: 'var(--color-bg)' }}>
      <Canvas
